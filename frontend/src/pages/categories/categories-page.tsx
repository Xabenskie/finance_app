...
export function CategoriesPage() {
	...
	const { role } = useAuth()
	const canManageCategories = role === 'manager' || role === 'admin'
	...
	const renderCategoryCard = (category: { id: string; name: string; type: string }) => {
					...
					<div className='flex items-center gap-3'>
						...
						<div>
							...
							<p className='text-xs text-muted-foreground'>
								{category.type === 'доход' ? 'Доходы' : 'Расходы'}
							</p>
						</div>
					</div>
					{canManageCategories && (
		...
		<div className='w-full mx-auto p-4 md:p-6 lg:p-8 space-y-6'>
			<div className='space-y-2'>
				...
				<p className='text-muted-foreground text-sm md:text-base'>
					{canManageCategories
						? 'Создавайте и управляйте категориями для классификации доходов и расходов'
						: 'Просмотр категорий для классификации доходов и расходов'}
				</p>
			</div>

			<div className={`grid grid-cols-1 ${canManageCategories ? 'lg:grid-cols-3' : ''} gap-6`}>
				{canManageCategories && (
				...
				<div className={`${canManageCategories ? 'lg:col-span-2' : ''} space-y-6`}>
					{/* Категории доходов */}
					<div className='border border-border/50 rounded-2xl p-6 bg-gradient-to-br from-background to-background/80 backdrop-blur-sm'>
						<div className='flex items-center gap-3 mb-6'>
